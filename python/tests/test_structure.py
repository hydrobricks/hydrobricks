...
def test_structure_graph_dot_groups_subbasin_stores():
    ...
    assert "cluster_subbasin" in dot
    assert "Subbasin (catchment level)" in dot
    # The glacier reservoirs are the basin-level stores.
    assert any(
        n.name == "glacier_area_icemelt_storage" and n.level == "subbasin"
        for n in with_glacier.nodes
    )

    # A plain HBV (no glacier) has no sub-basin store, hence no cluster.
    assert "cluster_subbasin" not in models.HBV96().get_structure_graph().to_dot()
