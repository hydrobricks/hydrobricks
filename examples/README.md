...
| `prevah_ticino.py` | The PREVAH-UniBE model on a 570-HRU, multi-cover, spatially-distributed catchment (Ticino-Bellinzona). Per-HRU field capacity via `set_spatial`; reproduces Fortran PREVAH (NSE ~0.96). Pure Python (meteo-zone forcing set per unit). |
