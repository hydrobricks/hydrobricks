...
class Observations(TimeSeries1D):
    ...
    def compute_reference_metric(self, metric, start_date=None, end_date=None,
        ...
        if mean_discharge:
            ...
            print("kge, ", he.kge_2012(df.mean_discharge.values, df.data.values, return_all=True))
            print("nse, ", he.nse(df.mean_discharge.values, df.data.values))

            print(df.data.values)
            print(df.mean_discharge.values)
            print(ref_metric)
            return ref_metric
