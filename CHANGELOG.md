...
- Adding the lateral connectivity of the hydro units to the project file (`hydro_units.connectivity`, a table from `Catchment.calculate_connectivity`), and always loading the `slope` column of a hydro units CSV: the lateral processes (e.g. `transport:snow_slide`) can now be used from a project or study file.
