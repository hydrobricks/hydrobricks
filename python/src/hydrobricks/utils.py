...
def create_forcing_file(
    path, hydro_units, time, forcing_data, forcing_variables, max_compression=False
):
    ...
    for index, unit in hydro_units.iterrows():
        ...
        if len(temp_lapse) == 1:
            units_temperature[:, index] = (
                ts["temperature"] + temp_lapse * (elevation - reference_alt) / 100
            )
            ...
            for m in range(12):
                ...
                units_temperature[month, index] = (
                    ts["temperature"][month]
                    + temp_lapse[m] * (elevation - reference_alt) / 100
                )
        else:
            raise ValueError(
                f"The temperature lapse should have a length of 1 or 12. Here: {len(temp_lapse)}"
            )

    return units_temperature


def spatialize_precip(
    ts,
    hydro_units,
    reference_alt,
    precip_corr_1,
    ...
    for index, unit in hydro_units.iterrows():
        ...
        if elevation < precip_corr_change:
            ...
            precip_below = ts["precipitation"] * (
                1 + precip_corr_1 * (precip_corr_change - reference_alt) / 100
            )
            units_precip[:, index] = precip_below * (
                1 + precip_corr_2 * (elevation - precip_corr_change) / 100
            )
    ...
    for index, unit in hydro_units.iterrows():
        ...
        if len(pet_gradient) == 1:
            if pet_gradient == 0:
                ...
                units_pet[:, index] = (
                    ts["pet"] + pet_gradient * (elevation - reference_alt) / 100
                )
            ...
            for m in range(12):
                ...
                units_pet[month, index] = (
                    ts["pet"][month]
                    + pet_gradient[m] * (elevation - reference_alt) / 100
                )
        else:
            raise ValueError(
                f"The temperature lapse should have a length of 1 or 12. Here: {len(pet_gradient)}"
            )
...
def mjd_to_datetime(mjd):
    ...
    for idx, _ in enumerate(year):
        date[idx] = datetime.datetime(
            year[idx], month[idx], day[idx], hour[idx], min[idx], 0, 0
        )
