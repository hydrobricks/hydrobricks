...
- Running the model on a sub-daily time step: `Model.setup()` now takes `time_step` and `time_step_unit` (`'day'`, `'hour'` or `'minute'`). The parameters and the process rates stay per day whatever the step, so a calibrated parameter set carries over unchanged; the forcing is an amount per step, so an hourly run takes hourly precipitation and PET totals. A step of 24 hours reproduces the daily run exactly. The formulations that are discrete by construction stay daily (the GR4J/GR6J/HBV unit hydrographs advance one slot per step, and the GR4J production store works on step volumes).
