...
- Adding a distributed PREVAH example (`examples/advanced/prevah_ticino.py`) on the Ticino-Bellinzona catchment (570 hydrotopes, open/forest/wetland covers, per-HRU field capacity via `set_spatial`, and PREVAH's monthly vegetation tables per cover via `apply_land_use`), with the data under `tests/files/catchments/ch_ticino_bellinzona/`. It runs the faithful configuration (vapour-density Hamon PET, albedo-reduced soil/canopy/snow evaporation, the PREVAH snow water release and the wet-surface evaporation) with the parameters of the original control file, and reproduces the reference Fortran PREVAH discharge (NSE ~0.98).
