...
class Plotter:
    ...
    This class provides methods for plotting time series comparisons, spatial
    distributions of model components across hydro units, and animated maps
    ...
    def plot_map_hydro_unit_value(
        ...
        boundary = Plotter._create_catchment_boundary(unit_ids_raster)
        ...
        # Plot
        cmap = Plotter._generate_cmap_blue()
    ...
    def _load_units_ids_raster(unit_ids_raster_path: str | Path) -> np.ndarray:
        ...
        if not HAS_RIOXARRAY:
            raise DependencyError(
                "rioxarray is required for some plotting operations.",
                package_name="rioxarray",
                operation="Plotter._load_units_ids_raster",
