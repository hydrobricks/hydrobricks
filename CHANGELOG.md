...
- Rejecting a parameter that is both spatial and monthly: the per-unit value takes precedence over the shared one that the monthly update writes, so the monthly variation would have been silently lost. `ParameterSet.set_spatial` and `set_monthly_values` now raise on the combination instead.
