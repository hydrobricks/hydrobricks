...
glacier_evolution.compute_lookup_table(catchment, update_width=UPDATE_WIDTH)
...
we_raw = pd.read_csv(working_dir / "details_glacier_we_evolution.csv", header=[0, 1, 2])
...
for i in range(0, len(areas_evol), 1):
    volume = np.sum(
        areas_evol.iloc[i, :].values * we_evol.iloc[i, :].values / (1000 * ICE_WE)
    )
