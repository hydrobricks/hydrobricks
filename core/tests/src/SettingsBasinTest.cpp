...
TEST(SettingsBasin, ParseNcFile) {
    ...
    EXPECT_TRUE(unitSettings.surfaceElements[0].name == "ground");
    EXPECT_TRUE(unitSettings.surfaceElements[0].type == "ground");
    EXPECT_FLOAT_EQ(unitSettings.surfaceElements[0].fraction, 0.80616301f);

    EXPECT_TRUE(unitSettings.surfaceElements[1].name == "glacier-ice");
    EXPECT_TRUE(unitSettings.surfaceElements[1].type == "glacier");
    EXPECT_FLOAT_EQ(unitSettings.surfaceElements[1].fraction, 0.08349901f);

    EXPECT_TRUE(unitSettings.surfaceElements[2].name == "glacier-debris");
    EXPECT_TRUE(unitSettings.surfaceElements[2].type == "glacier");
