...
- Adding `wetland` and `urban` (built-up) land covers available in all models as generic soil covers (distinct identity for extraction, labelling and per-cover parameters). The ESA WorldCover and CORINE land-cover presets now map built-up classes to `urban` and herbaceous wetlands to `wetland`. PREVAH additionally routes a fraction of the wetland input directly to the groundwater (PREVAH wet-surface, alias `wet_fraction`).
