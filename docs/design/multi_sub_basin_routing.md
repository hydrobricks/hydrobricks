...
| `muskingum_cunge` | `width` [m], `manning` | DONE. `K`, `X` recomputed each step from slope, width, Manning and the discharge (Manning depth, kinematic celerity 5/3·v, Cunge's X). Needs no calibration. The reach is divided into sub reaches routed in series (Ponce and Theurer, Δx ≤ (c·Δt + q/(S₀·c))/2), the count fixed at the first step from the reference discharge and capped at 50. |
...
| 5 | DONE. `Catchment.delineate_subbasins` (`CatchmentNetwork` in `preprocessing/catchment_network.py`): outlets snapped to the accumulation-based streams or to a river shapefile, nested delineation, implicit catchment-outlet subbasin, unit assignment by majority or split, reach length and slope from the DEM; `save_subbasin_ids_raster`; example `delineate_subbasins.py`. Reach length from the shapefile geometry DONE 2026-09-23 (`reach_lengths='auto'|'flow_path'`): the lines are a graph, the reach is the shortest path along it between the two outlet points, the flow path is kept when no path is found or the ratio to it leaves [0.5, 3]. | Example produces the Sitter tree from the two outlet points; reach lengths match the shapefile within 1 %. |
